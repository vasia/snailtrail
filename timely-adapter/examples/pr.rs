...
fn main() {
    ...
    let nodes: usize = args.next().expect("missing #nodes").parse::<usize>().unwrap();
    let edges: usize = args.next().expect("missing #edges").parse::<usize>().unwrap();
    ...
    timely::execute_from_args(args.clone().into_iter(), move |worker| {
        ...
        println!("w{} {:?}\tEpoch 0 complete", index, timer.elapsed());
        println!("w{} [st] closed times before: {:?}", index, input.time());
        if let Some(timely_logger) = &timely_logger {
            timely_logger.log(TimelyEvent::Text(format!(
        ...
        for i in 1 .. 1000 {
            ...
            println!("w{} {:?}\tRound {} complete", index, timer.elapsed(), i);
            println!("w{} [st] closed times before: {:?}", index, input.time());
            if let Some(timely_logger) = &timely_logger {
                timely_logger.log(TimelyEvent::Text(format!(
                    "[st] closed times before: {:?}",
