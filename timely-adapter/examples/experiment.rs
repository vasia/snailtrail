...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), move |worker| {
        ...
        input.send((Duration::new(0,0), 0, Operates(OperatesEvent {id: 0, name: "DEBUG".to_string(), addr: vec![0]})));
        input.send((Duration::new(0,0), 0, Operates(OperatesEvent {id: 0, name: "DEBUG".to_string(), addr: vec![0]})));
        input.send((Duration::new(0,0), 0, Operates(OperatesEvent {id: 0, name: "DEBUG".to_string(), addr: vec![0]})));
        input.send((Duration::new(0,0), 0, Operates(OperatesEvent {id: 0, name: "DEBUG".to_string(), addr: vec![0]})));
        input.send((Duration::new(0,0), 0, Operates(OperatesEvent {id: 0, name: "DEBUG".to_string(), addr: vec![0]})));
        ...
        input.send((Duration::new(0,5), 1, Schedule(ScheduleEvent {id: 1, start_stop: StartStop::Start})));
        input.advance_to(Duration::new(2,0));
        while probe.less_than(&input.time()) {
            worker.step();
        }

        input.send((Duration::new(2,0), 1, Schedule(ScheduleEvent {id: 5, start_stop: StartStop::Start})));
        input.advance_to(Duration::new(3,0));

        input.send((Duration::new(3,0), 1, Schedule(ScheduleEvent {id: 7, start_stop: StartStop::Start})));
        input.advance_to(Duration::new(4,0));

        input.send((Duration::new(4,0), 2, Schedule(ScheduleEvent {id: 9, start_stop: StartStop::Start})));
        input.advance_to(Duration::new(5,0));

        input.send((Duration::new(5,0), 2, Schedule(ScheduleEvent {id: 2, start_stop: StartStop::Start})));
